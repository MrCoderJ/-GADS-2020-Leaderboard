...
object GoogleFormClient{
    private const val DEV_SERVER = "https://docs.google.com/forms/d/e/"
    ...
    private fun getOkHttpClient(ctx: Context): OkHttpClient {
        return OkHttpClient.Builder().addInterceptor(GoogleFormClient.getLoggingInterceptor()).addInterceptor(
            GoogleFormClient.getAuthInterceptor(ctx)
    ...
    fun getClient(ctx: Context): RetrofitInterface {
        return Retrofit.Builder().baseUrl(GoogleFormClient.DEV_SERVER).client(
            GoogleFormClient.getOkHttpClient(
                ctx
            )
        )
